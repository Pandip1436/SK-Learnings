        ...
        {programs.map((p, i) => (
          <Reveal key={p.slug} delay={i * 100}>
            <article className="lift glow-border group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0c1226]/60 p-8 backdrop-blur-xl transition hover:bg-[#0c1226]/80 sm:p-12">
              {/* Corner Accent Glow */}
              <div className={`absolute -right-20 -top-20 h-64 w-64 rounded-full opacity-0 blur-[80px] transition-opacity group-hover:opacity-20 ${
                p.accent === 'cyan' ? 'bg-cyan-500' : p.accent === 'violet' ? 'bg-violet-500' : p.accent === 'sky' ? 'bg-sky-500' : 'bg-emerald-500'
              }`} />

              <div className="flex flex-wrap items-start justify-between gap-6">
                <div>
                  <p className={`font-display text-xs font-semibold uppercase tracking-[0.22em] ${
                    p.accent === 'cyan' ? 'text-cyan-300' : p.accent === 'violet' ? 'text-violet-300' : p.accent === 'sky' ? 'text-sky-300' : 'text-emerald-300'
                  }`}>
                    Program 0{i + 1}
                  </p>
                    ...
                    {p.highlights.map((h) => (
                      <li key={h} className="flex items-start gap-3 text-sm text-parchment/80">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`mt-0.5 shrink-0 ${
                          p.accent === 'cyan' ? 'text-cyan-400' : p.accent === 'violet' ? 'text-violet-400' : p.accent === 'sky' ? 'text-sky-400' : 'text-emerald-400'
              ...
              <div className="mt-10 flex pt-8 border-t border-white/5">
                <Link
                  to="/contact"
                  className={`group inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-semibold text-white transition-all hover:scale-105 ${
                    p.accent === 'cyan' ? 'bg-cyan-500/10 border border-cyan-500/20 hover:bg-cyan-500/20' : 
