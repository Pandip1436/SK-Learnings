  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `text-sm tracking-wide transition-all px-4 py-2 rounded-xl ${
                  isActive
                    ? "text-cyan-300 font-semibold bg-cyan-400/10 border border-cyan-400/20 shadow-[0_0_20px_-5px_rgba(34,211,238,0.2)]"
        ...
        <Link
          to="/contact"
          className="hidden md:inline-flex items-center gap-2 rounded-full bg-linear-to-r from-cyan-400 to-sky-500 px-6 py-2.5 text-sm font-semibold text-white shadow-[0_0_20px_-5px_rgba(34,211,238,0.5)] transition hover:scale-105"
        >
          Book a free demo
          ...
          <nav className="flex flex-col px-6 py-4">
            {navLinks.map((link) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `py-3 text-base ${
                    isActive ? "text-cyan-300 font-semibold" : "text-parchment/70"
                  }`
                }
              >
                {link.label}
